...
import { Booking, Package, Profile } from '../types';
...
export function BookingManagement({ showToast }: BookingManagementProps) {
  ...
  const bookingConfirmedHtml = (booking: BookingWithDetails) => {
    ...
    return `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <h2 style="color: #1E40AF;">Dear ${booking.profile?.full_name || 'User'},</h2>
  ...
  const bookingCancelledHtml = (booking: BookingWithDetails, reason: string) => {
    ...
    return `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <h2 style="color: #B91C1C;">Dear ${booking.profile?.full_name || 'User'},</h2>
  ...
  const paymentConfirmedHtml = (booking: BookingWithDetails) => {
    ...
    return `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <h2 style="color: #059669;">Dear ${booking.profile?.full_name || 'User'},</h2>
          ...
          <div className="flex space-x-2">
            {(['all', 'pending', 'pending_payment', 'confirmed', 'cancelled'] as const).map((status) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg transition-colors ${filterStatus === status ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
              >
                  ...
                  return (
                    <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-gray-900">{booking.profile?.full_name || (booking as any).name || 'Unknown'}</div>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-700">
                        {createdDate ? new Date(createdDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}
                        <div className="text-xs text-gray-500">
                          {createdDate ? new Date(createdDate).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : ''}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700">
            ...
            <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-fade-in-up">
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2">
                      <p><span className="text-gray-500">Name:</span> <span className="font-semibold">{selectedBooking.profile?.full_name || (selectedBooking as any).name || 'Unknown'}</span></p>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2">
                      ...
                      <p><span className="text-gray-500">Price Per Head:</span> ₹{selectedBooking.package?.price_per_head || selectedBooking.package?.price_per_head || 0}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                  <h4 className="font-bold text-gray-800 mb-3">Group Details ({(selectedBooking as any).number_of_members || '0'})</h4>

                  {(selectedBooking as any).adult_males !== undefined ? (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm font-semibold text-blue-800">Adults</p>
                        <ul className="list-disc pl-5 text-sm text-gray-700">
                          <li>Males: {(selectedBooking as any).adult_males ?? 0}</li>
                          <li>Females: {(selectedBooking as any).adult_females ?? 0}</li>
                          <li>Couples: {(selectedBooking as any).couples ?? 0} ({((selectedBooking as any).couples ?? 0) * 2} people)</li>
                        </ul>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-blue-800">Children</p>
                        <ul className="list-disc pl-5 text-sm text-gray-700">
                          <li>5-8 Years: {(selectedBooking as any).child_5_to_8 ?? 0}</li>
                          <li>Under 5: {(selectedBooking as any).child_under_5 ?? 0}</li>
                        </ul>
                      </div>
                    </div>
                  ) : (
                    ...
                    ) : <p className="text-sm text-gray-500">No detailed member info.</p>
                  )}
                </div>
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      ...
                      {selectedBooking.advance_utr && <p className="text-gray-700">UTR: {selectedBooking.advance_utr}</p>}
                    </div>
                  ...
                  {((selectedBooking as any).advance_receipt_url || (selectedBooking as any).payment_screenshot) && (
                    <div className="pt-4">
                      <p className="font-medium text-gray-500 mb-2">Payment Screenshot:</p>
                      <div className="relative group">
                        <img
                          src={getReceiptPublicUrl(selectedBooking.advance_receipt_url) || (selectedBooking as any).payment_screenshot || ''}
